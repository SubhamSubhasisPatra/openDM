...
const MultipartProgressBar = ({numParts}) => {
  ...
  useEffect(() => {
    if (progressValues.length > 0) {
      const interval = setInterval(() => {
        setProgressValues(prevProgressValues => prevProgressValues.map(value => Math.min(value + Math.random() * 15, 100)));
      }, 250);
  ...
  return (<div className="w-full mt-2">
    {!overallComplete ? (<div className="flex w-full">
      {progressValues.map((progress, index) => (<div
        key={index}
    ...
    </div>) : (<div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
      <div
        className="h-full bg-green-500 transition-all duration-500"
        style={{width: '100%'}}
      ></div>
